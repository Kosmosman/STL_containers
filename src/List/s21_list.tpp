...
template <typename value_type> list<value_type>::list(list &&l) : list() {
  *this = std::move(l);
}
...
template <typename value_type> void list<value_type>::erase(ListIterator pos) {
  Node *node = pos.ptr_;
...
template <typename value_type> void list<value_type>::index_alignment() {
  if (end_) {
    end_->next_ = head_;
    end_->prev_ = tail_;
    end_->value_ = size();
